...
- "Posts" (the third thing in this README's first sentence) had no automated source until today — `hirogari collect` now also checks Hacker News (`hn.algolia.com`, no auth) for direct-link submissions of the repo, real points threshold applied. Verified live against 7 real projects: `django/django` had 6 real hits, `pypa/pip` 3, `numpy/numpy` 3, `psf/requests` and `tiangolo/fastapi` 1 each, `pytest-dev/pytest` and `pallets/click` 0 — every hit found across all seven predates pypistats' ~180-day window, some by a decade. It's a deliberately narrow, precision-over-recall source (a story that mentions a project without linking directly to its GitHub repo is invisible to it) — see [notes/2026-09-12-hn-posts-source.md](notes/2026-09-12-hn-posts-source.md) for what that trades away and why.
